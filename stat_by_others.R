...
if (new){
  ...
  df_table_occ_country_2<-data.frame()
...
if (!new){
  ...
  df_table_occ_country_2<-read.table(sprintf("%s/df_table_occ_country_2.csv", folder), head=T, sep=",", stringsAsFactors = F)
...
for (genus in df_genus$genus){
  ...
  table_occ_road_basis<-data.frame(table(df_occ_road$road_cut2, df_occ_road$basisOfRecord))
  ...
  threshold<-2500
  df_occ_country_2<-df_occ[which(!is.na(df_occ$countryCode)),]
  if (nrow(df_occ_country_2)==0){
    next()
  }
  df_occ_country_2$lower_th<-F
  if (nrow(df_occ_country_2[which(df_occ_country_2$road<=threshold),])>0){
    df_occ_country_2[which(df_occ_country_2$road<=threshold),]$lower_th<-T
  }
  
  table_occ_country<-data.frame(table(df_occ_country_2$countryCode, df_occ_country_2$lower_th))
  table_occ_country<-table_occ_country[which(table_occ_country$Var1!=""),]
  table_occ_country_2<-merge(table_occ_country[which(table_occ_country$Var2==TRUE),],
                             table_occ_country[which(table_occ_country$Var2==FALSE),],
                             by="Var1", all=T)
  table_occ_country_2<-table_occ_country_2[, c("Var1", "Freq.x", "Freq.y")]
  
  colnames(table_occ_country_2)<-c("countryCode", "in_th", "out_th")
  table_occ_country_2[is.na(table_occ_country_2)]<-0
  table_occ_country_2$all<-table_occ_country_2$in_th+table_occ_country_2$out_th
  table_occ_country_2<-table_occ_country_2[order(-table_occ_country_2$all),] 
  table_occ_country_2$genus<-genus
  if (nrow(df_table_occ_country_2)==0){
    df_table_occ_country_2<-table_occ_country_2
  }else{
    df_table_occ_country_2<-rbind(df_table_occ_country_2, table_occ_country_2)
  }
  
  
  #What % of records falls within 2.5km of a road inside a Protected area vs outside a protected area
  
  df_occ_pa<-df_occ_country_2[which(!is.na(df_occ_country_2$pa)),]
  if (nrow(df_occ_pa)==0){
    next()
  }
  ...
  write.table(df_table_occ_country_2, sprintf("%s/df_table_occ_country_2.csv", folder), row.names=F, sep=",")
